...
#     return [rank + 1 for rank in fail_stage]
    ...
    fail_ratio = {}
    players = len(stages) # 스테이지에 도달한 플레이어 수

    for i in range(1,N+1): # 스테이지 번호 for문
        if players != 0: # ls가 0인 경우에
            fail_ratio[i] = stages.count(i) / players  # 각각의 스테이지와 실패율을 계산해서 딕셔너리에 추가
            players -= stages.count(i) # 다음 스테이지에 대해서 실패한 플레이어 수 빼기
        else: #
            fail_ratio[i] = 0
    return sorted(fail_ratio, key=lambda i: fail_ratio[i], reverse=True)
